  ...
  return (
    <article className="lesson-card group bg-white">
      <button
        ...
        className="grid w-full grid-cols-[96px_minmax(0,1fr)] gap-4 p-3 text-left outline-none focus-visible:ring-2 focus-visible:ring-[#20211F] focus-visible:ring-inset sm:grid-cols-[118px_minma..."
      >
        ...
        <div className="min-w-0 py-0.5">
          ...
          <h3 className="mt-2 line-clamp-2 font-display text-[1.35rem] leading-[1.02] tracking-[-0.02em] text-[#232421] transition-colors group-hover:text-[#53565A] sm:text-[1.55rem]">{(video as any).title}</h3>
          <p className="mt-1.5 truncate text-xs text-[#74777A]">{(video as any).channel} · {(video as any).level}</p>
...
export default function Home() {
  ...
  const visibleCatalog = useMemo(
    () => filterCatalog("All", normalizedSearch),
    [normalizedSearch],
  );
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#FBFBFA] text-[#20211F]">
      ...
      <main>
        <section className="relative isolate mx-auto flex min-h-[640px] max-w-[1440px] items-center justify-center px-5 pb-24 pt-12 sm:min-h-[690px] sm:px-8 lg:min-h-[740px] lg:px-12 lg:pb-32 lg:[...]
          ...
          <div className="relative z-10 w-full max-w-4xl text-center">
            ...
            <p className="mx-auto mt-7 max-w-2xl text-base leading-7 text-[#6B6D6B] sm:text-lg sm:leading-8">Choose a useful explanation from a small, structured shelf—or bring a public video l[...]</p>
            ...
            <div className="mx-auto mt-9 max-w-2xl border border-[#DADBD9] bg-white p-1.5 shadow-[0_18px_38px_rgba(45,46,42,0.05)] sm:mt-10 sm:p-2">
              <div className="flex items-center">
                ...
                />
                <Button type="button" onClick={handleHeroSearch} className="h-10 shrink-0 bg-[#252624] px-3.5 text-xs font-semibold text-white hover:bg-[#515350] active:scale-[0.97] sm:h-11 sm:px[...]">
              ...
              <div>
                <div className="flex flex-col gap-4 border-b border-[#DDE0E3] pb-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <label className="relative block w-full sm:w-[285px]">
                    ...
                    <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="h-9 w-full border border-[#D8DBDE] bg-white pl-9 pr-8 text-sm outline-none focus:b..." />
                    {searchTerm && <button type="button" onClick={() => setSearchTerm("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#8D9095] hover:text-[#2B2C2B]" aria-label="Cl...">{<X className="h-4 w-4" />}</button>}
                  </label>
                </div>

                {importedVideo && (
                  <button type="button" onClick={() => setActiveVideo(importedVideo)} className="mt-5 flex w-full items-center justify-between gap-4 border border-[#D9DCE0] bg-white px-4 py-3 tex...">
                    <span className="flex min-w-0 items-center gap-3"><Check className="h-4 w-4 shrink-0" /><span className="min-w-0"><span className="block text-xs text-[#85888C]">Recent direct...</span></span></span>
                ...
                {hasDisplayedLessons ? (
                  <div className="mt-6 space-y-6">
                    {showFocusPick && (
                      <button type="button" onClick={() => setActiveVideo(focusPick)} className="group grid w-full overflow-hidden border border-[#D8DBDE] bg-white text-left outline-none transiti...">
                        ...
                        <div className="flex flex-col justify-center p-6 sm:p-8 lg:p-10">
                          ...
                          <div className="mt-7 flex items-center justify-between border-t border-[#E1E3E5] pt-4 text-xs text-[#70737A]"><span>{focusPick.channel} · {focusPick.duration}</span><sp...
                        </div>
                      </button>
                    )}
                    <div className="grid gap-3 sm:grid-cols-2">
                      {displayedLessons.map((video, index) => <VideoLesson key={(video as any).id} video={video} index={index + (showFocusPick ? 1 : 0)} onOpen={setActiveVideo} />)}
                    </div>
                  </div>
                ) : (
                  <div className="mt-6 border border-dashed border-[#CDD1D5] bg-white px-6 py-16 text-center">
                    ...
                    <h3 className="mt-4 font-display text-3xl">{liveSearch.isFetching ? "Looking beyond the shelf." : shouldUseLiveSearch ? "No lesson surfaced for that yet." : "Nothing matching..."}</h3>
                    <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[#74777D]">{shouldUseLiveSearch ? liveSearch.data?.message || "The optional public providers did not return a match.[..." : "Try a different search"}</p>
        ...
        <section className="mx-auto grid max-w-[1440px] gap-10 px-5 py-24 sm:px-8 sm:py-32 lg:grid-cols-[1fr_minmax(390px,0.82fr)] lg:gap-20 lg:px-12 lg:py-40">
          ...
          <div className="self-end lg:pb-1">
            <p className="max-w-md text-base leading-8 text-[#6B6E6C]">Paste a public YouTube or Vimeo URL when you already know what you need. Lesson Ledger reads its metadata, opens a clean pla...</p>
            <Button type="button" onClick={() => setImportOpen(true)} variant="outline" className="mt-7 h-11 border-[#BFC3C7] bg-white px-4 text-sm font-semibold text-[#292A28] hover:bg-[#F0F1F2]">Open a link</Button>
          </div>
        </section>
        ...
        <footer className="bg-[#20211F] px-5 py-12 text-[#F6F6F2] sm:px-8 sm:py-14 lg:px-12">
          <div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-10 sm:flex-row sm:items-end">
            <div>
              <div className="flex items-center gap-2.5"><span className="flex h-8 w-8 items-center justify-center border border-white/40 font-display text-lg italic">L</span><span className="font-..."></span></div>
            ...
            <div className="p-6 sm:p-8">
              <label className="block">
                ...
                <div className="flex border border-[#D5D7D4] bg-white focus-within:border-[#292A28] focus-within:ring-1 focus-within:ring-[#292A28]">
                  ...
                  <input value={importUrl} onChange={(event) => { setImportUrl(event.target.value); setImportError(""); }} onKeyDown={(event) => event.key === "Enter" && void importVideo()} className="h-9 w-full bg-transparent px-3 text-sm outline-none" placeholder="https://" />
                </div>
              </label>
              ...
              <div className="mt-5 flex flex-col-reverse gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button type="button" onClick={() => void importVideo()} disabled={isImporting} className="h-10 bg-[#252624] px-4 text-sm font-semibold text-white hover:bg-[#515350] active:scale-[0.97]">{isImporting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ArrowRight className="mr-2 h-4 w-4" />} Open lesson</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
          ...
          <DialogContent className="max-w-5xl overflow-hidden border-[#222321] bg-[#20211F] p-0 text-white shadow-[0_28px_80px_rgba(28,29,27,0.32)]">
            {activeVideo && <>
              ...
              <div className="grid lg:grid-cols-[minmax(0,1.7fr)_minmax(260px,0.8fr)]">
                <div className="relative aspect-video overflow-hidden bg-black"><iframe className="h-full w-full" src={`${(activeVideo as any).embedUrl}&modestbranding=1`} title={(activeVideo as any).title} sandbox="allow-same-origin allow-scripts allow-presentation" /></div>
                <div className="flex flex-col p-6 sm:p-8">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/50">{(activeVideo as any).topic} · {(activeVideo as any).level}</p>
                  <h2 className="mt-4 font-display text-4xl leading-[0.92] tracking-[-0.04em]">{(activeVideo as any).title}</h2>
                  <p className="mt-3 text-sm text-white/62">{(activeVideo as any).channel} · {(activeVideo as any).duration}</p>
                  <p className="mt-5 text-sm leading-6 text-white/72">{(activeVideo as any).note}</p>
